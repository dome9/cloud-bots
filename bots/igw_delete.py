            ...
            for instance in reservation["Instances"]:
                ...
                for interface in instance['NetworkInterfaces']:
                    for ips in interface['PrivateIpAddresses']:
                        try:
                            print("Dome9 Cloud bots - igw_delete.py - ips {}".format(ips['Association']['PublicIp']))
                            print("Dome9 Cloud bots - igw_delete.py - instance id : {}".format(instance_id))
                            instances_to_turn_off.append(instance_id)
                        except:
                            continue

        if instances_to_turn_off:
            print("Dome9 Cloud bots - igw_delete.py - turning instances off")
            ...
            if responseCode >= 400:
                ...
                while instance.state['Name'] not in 'stopped':
                    print("Dome9 Cloud bots - igw_delete.py - Sleeping while waiting for instances to turn off (usually about 45 seconds)")
                    sleep(5)
                    instance.load()
                print("Dome9 Cloud bots - igw_delete.py - Instances are fully shut down. Continuing")
